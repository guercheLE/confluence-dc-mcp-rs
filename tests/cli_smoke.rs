...
fn generated_server(args: &[&str]) -> Output {
    Command::new(env!("CARGO_BIN_EXE_confluence-dc-mcp"))
        .args(args)
        .env("CONFLUENCE_DC_MCP_URL", "http://127.0.0.1")
        .env("CONFLUENCE_DC_MCP_AUTH_METHOD", "basic")
        .env_remove("CONFLUENCE_DC_MCP_API_VERSION")
        .env_remove("CONFLUENCE_DC_MCP_TRANSPORT")
        // Isolates the config cascade's global-config layer
        // (`~/.confluence-dc-mcp/config.yml`, resolved via
...
fn version_help_versions_and_config_are_available() {
    ...
    assert!(stdout(&versions).contains("10.2.14"));
    ...
    assert_eq!(config["api_version"], "10.2.14");
...
fn invalid_commands_operations_and_arguments_fail_without_network_calls() {
    ...
    let invalid_json = generated_server(&["call", "getAccessModeStatus", "--args", "not-json"]);
...
fn stdio_and_helper_binaries_cover_their_bootstrap_and_error_paths() {
    ...
    let healthy = Command::new(env!("CARGO_BIN_EXE_confluence-dc-mcp-healthcheck"))
    ...
    let unhealthy = Command::new(env!("CARGO_BIN_EXE_confluence-dc-mcp-healthcheck"))
        .current_dir(empty.path())
        .output()
        .unwrap();
    assert!(!unhealthy.status.success());

    let populate = Command::new(env!("CARGO_BIN_EXE_confluence-dc-mcp-populate-embeddings"))
    ...
    .unwrap();
    let populated = Command::new(env!("CARGO_BIN_EXE_confluence-dc-mcp-populate-embeddings"))
        .arg(&store_copy)
        .output()
        .unwrap();
...
fn http_command_serves_health_and_shuts_down_cleanly() {
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_confluence-dc-mcp"))
        .args([
            "http",
            "--host",
            "127.0.0.1",
