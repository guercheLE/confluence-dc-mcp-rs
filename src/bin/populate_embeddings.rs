...
use anyhow::Context;
use confluence_dc_mcp::data::store::{VERSION_STORE_FILES, open_store_read_write};
use confluence_dc_mcp::services::embedding_service::embed;
...
/// anywhere. An explicit path argument still targets exactly that one
/// file, for a fast single-store re-run. `--all` is kept as an accepted
/// synonym for the (now-default) full-ledger behavior, so existing callers
