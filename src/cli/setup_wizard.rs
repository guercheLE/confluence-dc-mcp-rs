...
const LOCAL_CONFIG_FILE: &str = "confluence-dc-mcp.config.yml";
const CONFIG_DIR_NAME: &str = ".confluence-dc-mcp";
...
/// `~/.confluence-dc-mcp/config.yml` if they pick "global".
    ...
    match selection {
        ...
        "Write a config.yml file" => {
            let tier_choices = vec![
                "Local (this directory only — ./confluence-dc-mcp.config.yml)",
                "Global (all runs for this user — ~/.confluence-dc-mcp/config.yml)",
            ];
...
pub async fn run_setup_wizard() -> anyhow::Result<()> {
    ...
    config_fields.insert("url".to_string(), env["CONFLUENCE_DC_MCP_URL"].clone());
    config_fields.insert(
        "auth_method".to_string(),
        env["CONFLUENCE_DC_MCP_AUTH_METHOD"].clone(),
    );
    config_fields.insert(
        "api_version".to_string(),
        env["CONFLUENCE_DC_MCP_API_VERSION"].clone(),
    );
    config_fields.insert(
        "transport".to_string(),
        env["CONFLUENCE_DC_MCP_TRANSPORT"].clone(),
    );
