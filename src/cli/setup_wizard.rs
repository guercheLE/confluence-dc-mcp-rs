...
/// reads `./confluence-dc-mcp.config.yml` (local) or
/// `~/.confluence-dc-mcp/config.yml` (global).
...
async fn prompt_persistence(
    ...
    let choices = vec![
        "Write a .env file",
        "Write a config.yml file (global — ~/.confluence-dc-mcp/config.yml, read on every invocation on this machine)",
        "Write a config.yml file (local — ./confluence-dc-mcp.config.yml, read only from this directory)",
...
async fn persist_selection(
    ...
    let global_config = confluence_dc_mcp::core::credential_storage::resolve_home_dir()
        .join(".confluence-dc-mcp/config.yml");
...
async fn persist_selection_at(
    ...
    match selection {
        ...
        s if s.starts_with("Write a config.yml file (local") => {
            write_config_yaml(
                &local_dir.join("confluence-dc-mcp.config.yml"),
                config_fields,
            )
            .await?;
        }
...
fn build_runtime_settings(
    ...
    let config_fields = serde_json::json!({
        "url": env.get("CONFLUENCE_DC_MCP_URL"),
        "auth_method": env.get("CONFLUENCE_DC_MCP_AUTH_METHOD"),
        "api_version": env.get("CONFLUENCE_DC_MCP_API_VERSION"),
        "transport": env.get("CONFLUENCE_DC_MCP_TRANSPORT"),
    });
...
mod tests {
    ...
    fn runtime_settings_keep_config_fields_and_filter_ephemeral_oauth_values() {
        ...
        assert_eq!(env["CONFLUENCE_DC_MCP_CLIENT_ID"], "client");
        assert_eq!(env["CONFLUENCE_DC_MCP_AUTHORIZATION_CODE"], "spent-code");
    }
    ...
    async fn file_persistence_writes_env_local_yaml_and_global_yaml() {
        ...
        assert!(local_dir.join("confluence-dc-mcp.config.yml").is_file());
