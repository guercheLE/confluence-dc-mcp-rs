...
impl McpifyServer {
    ...
    #[tool(
        description = "Semantic search for Confluence Data Center operations using a natural-language query."
    )]
    async fn search(
        &self,
    ...
    #[tool(
        description = "Return the schema, path, method, and documentation for a specific Confluence Data Center operationId."
    )]
    ...
    #[tool(
        description = "Validate arguments, invoke a live Confluence Data Center API operation, and validate the response."
    )]
    async fn call(
        &self,
...
mod tests {
    ...
    async fn search_and_get_return_mcp_content_envelopes() {
        ...
        let operation_id = {
            let conn = cached_store_connection("10.2.14").unwrap();
