...
/// (`./confluence-dc-mcp.config.yml`) -> home file
/// (`~/.confluence-dc-mcp/config.yml`) -> system file
/// (`/etc/confluence-dc-mcp/config.yml`) -> install-dir file -> built-in
...
pub fn load_config(cli_flags: Map<String, Value>) -> Result<Config, McpifyError> {
    ...
    let layers = [
        ...
        read_yaml_if_exists(&PathBuf::from("/etc/confluence-dc-mcp/config.yml")),
