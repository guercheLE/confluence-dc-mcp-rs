...
use confluence_dc_mcp::core::component_registry::ComponentRegistry;
use confluence_dc_mcp::core::config_manager::load_config;
use confluence_dc_mcp::core::config_schema::Transport;
use confluence_dc_mcp::core::health_check_manager::HealthCheckManager;
use confluence_dc_mcp::core::logger::init_logging;
use confluence_dc_mcp::core::mcp_server::{McpifyServer, connect_stdio};
use confluence_dc_mcp::core::otel;
use confluence_dc_mcp::core::shutdown_handler::{install_shutdown_handlers, on_shutdown};
use confluence_dc_mcp::data::store::{open_store, resolve_store_path};
use confluence_dc_mcp::http::server::start_http_server;
use confluence_dc_mcp::http::types::HttpServerConfig;

#[derive(Parser)]
#[command(
    name = "confluence-dc-mcp",
    about = "Confluence Data Center MCP server",
    version
)]
struct Cli {
    #[command(subcommand)]
...
#[derive(Subcommand)]
enum Command {
    /// Interactively configure Confluence Data Center credentials and connection settings
    Setup,
...
async fn run_harness_server() -> anyhow::Result<()> {
    ...
    let (otel_layer, otel_provider) = match otel::build_layer("confluence-dc-mcp") {
    ...
    if config.transport == Transport::Http {
        ...
        let (header_location, header_name) =
            confluence_dc_mcp::auth::auth_manager::header_location_for(config.auth_method);
...
async fn main() -> anyhow::Result<()> {
    ...
    let result = match cli.command {
        ...
        Command::Start => {
            ...
            unsafe {
                std::env::set_var("CONFLUENCE_DC_MCP_TRANSPORT", "stdio");
            }
        ...
        Command::Http {
            ...
            // spawns any concurrent work that would also touch env vars.
            unsafe {
                std::env::set_var("CONFLUENCE_DC_MCP_TRANSPORT", "http");
                if let Some(host) = &host {
                    std::env::set_var("CONFLUENCE_DC_MCP_HOST", host);
                }
                if let Some(port) = port {
                    std::env::set_var("CONFLUENCE_DC_MCP_PORT", port.to_string());
                }
                if let Some(cors_allow) = &cors_allow {
                    std::env::set_var("CONFLUENCE_DC_MCP_CORS_ALLOW", cors_allow);
                }
            }
            run_harness_server().await
        }
