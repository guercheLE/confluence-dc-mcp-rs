...
Exposes exactly 3 tools — `search`, `get`, `call` — backed by an embedded semantic database (`mcp_store.db`), so an LLM never needs the full API surface in context. Also exposes [guided MCP prompts](#workflows-mcp-prompts) for common multi-step Confluence tasks, starting with `confluence_workflow`.
...
Start with `confluence_workflow` (optional argument: `goal`, a plain-language description of what you're trying to do). It routes to one of:

| Prompt | Covers |
|---|---|
| `confluence_workflow_spaces` | Space lifecycle: create, update, delete, archive, restore, categories, color scheme. |
| `confluence_workflow_content` | Pages/blog posts: create, update, delete, hierarchy, history, body-format conversion. |
| `confluence_workflow_attachments` | Upload, list, update, delete, move attachments. |
| `confluence_workflow_labels` | Labels on content and spaces. |
| `confluence_workflow_properties` | Key/value metadata on content and spaces. |
| `confluence_workflow_permissions_restrictions` | Content-level restrictions vs. space/global permission grants. |
| `confluence_workflow_users_groups` | User and group lifecycle, membership. |
| `confluence_workflow_search_cql` | CQL content search vs. general entity search. |
| `confluence_workflow_watches` | Watch subscriptions on content and spaces. |
| `confluence_workflow_backup_restore` | Site- or space-scoped backup/restore jobs. |
| `confluence_workflow_webhooks` | Webhook lifecycle and diagnostics. |
| `confluence_workflow_admin_diagnostics` | Reindexing, cluster status, audit records, and other operational signals. |
| `confluence_workflow_space_provisioning` | Composite: set up a brand-new team space end-to-end (create, permission, categorize, seed content). |
| `confluence_workflow_user_lifecycle` | Composite: onboard or offboard a user across accounts, groups, permissions, and watches. |
...
# 1. prompts/list  -> discover confluence_workflow and its 14 sub-workflows
# 2. prompts/get   -> { "name": "confluence_workflow", "arguments": { "goal": "set up a new team space" } }
#    routes to confluence_workflow_space_provisioning
```
