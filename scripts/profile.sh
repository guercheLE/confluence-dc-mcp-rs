...
export CONFLUENCE_DC_MCP_URL="${CONFLUENCE_DC_MCP_URL:-http://127.0.0.1}"

export CONFLUENCE_DC_MCP_AUTH_METHOD="${CONFLUENCE_DC_MCP_AUTH_METHOD:-basic}"


profile_query="${PROFILE_QUERY:-test query}"
profile_warm_seconds="${PROFILE_WARM_SECONDS:-30}"

cargo build --release --bin confluence-dc-mcp
cargo build --release --example profile_search

samply record --save-only --unstable-presymbolicate -o profile/cold-start.json.gz -- \
  ./target/release/confluence-dc-mcp search "$profile_query"
...
ready_file="$(mktemp "${TMPDIR:-/tmp}/confluence-dc-mcp-profile-ready.XXXXXX")"
rm -f "$ready_file"
warm_pid=""
samply_pid=""
cleanup() {
